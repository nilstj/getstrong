...
> 5. **`rewrite_gym_label` runs with `search_path = pg_catalog, public`, not `''`.** Found when the owner tried to apply the file: it raised `ERROR 42704: type "gym_problems" does not exist`. A `SET` clause applies for the whole call including nested ones, and a trigger function without its own `SET` inherits it — so updating `problems` fired `on_problem_crew_send` (045), whose `notify_crew_send()` declares `v_boulder gym_problems` unqualified, and plpgsql resolves DECLARE types on entry, before that function's early-return guard. The whole-branch review checked the guard's logic and concluded the rewrite was side-effect-free, which was right about notifications and missed that the function could not be entered at all. This would have aborted the backfill *and* every admin merge and rename in production. `pg_catalog, public` is still fixed and caller-proof, and every reference in the function's own body is already `public.`-qualified.
> 6. **A prerequisite guard runs first**, naming any of 060/071/079/080 that is unapplied, per 091's precedent.
> 7. **`drop policy if exists` before `create policy`**, so the file is re-runnable after a failed apply.
>
