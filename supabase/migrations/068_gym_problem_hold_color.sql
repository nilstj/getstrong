-- Give shared boulders a hold colour too (mirrors problems.hold_color, migration
-- 067). Add the column and capture it at creation. Reproduces migration 051's
-- create_gym_problem body and only adds p_hold_color; the old 6-arg signature is
-- dropped so the 7-arg version isn't an ambiguous overload.

alter table gym_problems add column if not exists hold_color text;

drop function if exists public.create_gym_problem(text, text, text, text, text, text);
...
begin
  ...
  -- first_logger points to the creator (preserved from migration 046/051).
